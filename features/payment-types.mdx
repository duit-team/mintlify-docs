...
<Steps>
  ...
  <Step title="Fill in the config panel">
    ...
    - **GKash**: **Merchant ID (CID)**, **Signature Key**, **Terminal ID** (optional), **Base URL**, and **Default Terminal Payment Method**
...
<AccordionGroup>
  <Accordion title="Which payment processors are supported?">
    ...
    - **GKash**: Online payment gateway for QR/web ordering today; SoftPOS terminal payments (tap-to-phone, DuitNow QR, etc.) are supported on the backend, with POS app support coming soon
  ...
  <Accordion title="Why is my payment processor not processing payments?">
    ...
    **For GKash:**
    1. Your Merchant ID (CID) and Signature Key are correct for the Base URL you've set (staging vs. production)
    2. Your merchant account is active and verified at GKash
    3. Your payment type is set to Active
...
<CardGroup cols={2}>
  ...
  <Card title="GKash Setup" icon="credit-card" href="/online-ordering/gkash-setup">Connect GKash for QR/online ordering</Card>
